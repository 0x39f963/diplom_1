...
validate-bench: ## Статическая проверка benchmark/gold датасетов
	$(PY) -m evals.validate_dataset
...
test: ## pytest
	pytest -q
gates: lint type validate-bench test ## Все гейты (ruff + mypy + validate-bench + pytest)
