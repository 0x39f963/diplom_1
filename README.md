...
выбирает протокол по онтологии. Подробности ниже в разделе «Обновление: текущий граф».
...
**Обновление: текущий граф**

Текущий граф вынесен отдельно: модель только заполняет анкету интента, план действий собирает код по
онтологии. Добавлены `nlu_preprocessor`, `intent_frame_parser`, `domain_selector`, компилятор в
`data_gather` и проверка покрытия. Так маршрут меньше зависит от свободного текста модели.

![Текущий граф агента](images/agent-graph-v2.svg)

Подробные схемы:

- [Оркестратор](images/orchestrator.svg) - показывает классификацию intent, high-recall override и
  маршрутизацию в ветку.
- [Разбор интента](images/intent-parsing.svg) - показывает, как запрос превращается в типизированную
  анкету.
- [Компилятор плана](images/compiler.svg) - показывает выбор протокола, сбор шагов и проверку
  покрытия.
- [Механизмы раунда](images/mechanisms.svg) - коротко фиксирует шесть изменений поведения агента.
